...
async function addStore(req, res) {
    ...
    try {
        ...
        const existingStore = await storeModel.findStoreByName(trimmedName);
        if (existingStore) {
        ...
        const newStore = await storeModel.createStore({ name: trimmedName, address: address.trim(), owner_id: owner_id || null });
        if (newStore) {
...
async function updateStore(req, res) {
    ...
    try {
        ...
        if (trimmedName && trimmedName !== store.name) {
            const existingStoreWithName = await storeModel.findStoreByName(trimmedName);
        ...
        const updated = await storeModel.updateStore(parseInt(id), { name: trimmedName, address: address ? address.trim() : address, owner_id });
        if (updated) {
