...
const StoreManagementPage = () => {
  ...
  const validateDialogForm = () => {
    const errors = [];
    const name = dialogFormData.name?.trim() || '';
    const address = dialogFormData.address?.trim() || '';
  ...
  const handleSaveStore = async () => {
    ...
    try {
      const dataToSave = {
        ...
        address: dialogFormData.address?.trim() || '',
      ...
      if (errorData?.errors && Array.isArray(errorData.errors)) {
        ...
        setDialogError(errorData.errors.join('\n'));
      } else {
        setDialogError(errorData?.message || err.message || 'Failed to save store.');
      }
    }
  };
